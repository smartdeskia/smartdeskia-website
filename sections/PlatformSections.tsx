...
export function HowItWorksSection() {
  ...
  return <section className="how sd-section" id="how-it-works"><div className="section-label"><span>02</span><p>HOW IT WORKS</p></div><div className="how-head"><h2>Natural conversations.<br /><em>Reliable follow-through.</em></h2><p>Sofia connects with the tools you already use and follows the exact rules of your business.</p></div><div className="cap-grid">{capabilities.map(item => <article key={item[0]}><span>{item[0]}</span><h3>{item[1]}</h3><p>{item[2]}</p></article>)}</div><DashboardPreview /></section>;
}
